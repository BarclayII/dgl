...
class HGTMessage(nn.Module):
    ...
    def forward(self, edges):
        ...
        h_t = edges.dst['h']            # float32[E, in_size]
        ...
        h_hat_t = h_t                   # float32[E, in_size]
        
        # Step 2. compute target queries, source keys, and source messages
        Q_t = self.Q(h_hat_t, tau_t).view(-1, self.n_heads, self.d_k)    # float32[E, n_heads, d_k]
...
class HGTConv(nn.Module):
    ...
    def forward(self, g, h):
        with g.local_scope():
            ...
            h_new = self.A(F.gelu(h_new), g.ndata['ntype'])
...
class HGT(nn.Module):
    def __init__(self, in_size, out_size, n_heads, n_etypes, n_ntypes, n_layers, n_classes, dropout=0.2):
        ...
        self.adapt_ws = TypedLinear(n_ntypes, in_size, out_size).to('cuda:0')
        self.convs = nn.ModuleList()
        self.norms = nn.ModuleList()
        for i in range(n_layers):
            device = torch.device('cuda:%d' % (i // 2))
            self.convs.append(HGTConv(out_size, out_size, n_heads, n_etypes, n_ntypes, dropout).to(device))
            self.norms.append(TypedLayerNorm(n_ntypes, out_size).to(device))
        self.dropout = nn.Dropout(dropout).to(device)
        self.classifier = nn.Linear(out_size, n_classes).to(device)
